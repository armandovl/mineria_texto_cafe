...
mensajes_mes_anyo %>% group_by(tipo, mes_anyo) %>% summarise(n = n()) %>%
  ...
  labs(title = "Número de mensajes publicados", x = "fecha de publicación",
...
palabras_comunes <- dplyr::intersect(mensajes_tidy %>% filter(tipo=="cafe_soluble") %>%
                                       ...
                                       select(token)) %>% nrow()
paste("Número de palabras comunes entre soluble y molido", palabras_comunes)
...
palabras_comunes <- dplyr::intersect(mensajes_tidy %>% filter(tipo=="cafe_grano") %>%
                                       select(token), mensajes_tidy %>% filter(tipo=="cafe_molido") %>%
                                       select(token)) %>% nrow()
paste("Número de palabras comunes entre grano y molido", palabras_comunes)
...
# Selección de los tipoes elonmusk y mayoredlee
mensajes_unpivot <- mensajes_unpivot %>% filter(tipo %in% c("cafe_grano",
                                                            "cafe_molido"))
# Se añade el total de palabras de cada tipo
